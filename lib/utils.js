...
var regexps = {
  ...
  positiveRe: /article|body|content|entry|hentry|page|pagination|post|post-content|document|article-post|post-article|text/i,
...
function filterCandidates(topCandidate, siblings, $) {
  ...
  siblings.each(function(index, elem) {
    ...
    if (!append && (type == 'p')) {
      ...
      if (len < 3 && type == 'p') return;

      if (len > 80 && linkDensity < 0.25 && type == 'p') {
        append = true;
      } else if (len < 80 && linkDensity == 0 && node.text().replace(regexps.trimRe, "").length > 0 && type == 'p') {
        append = true;
      }
    ...
    if (!append && (type == 'table')) {
      // add conditionals for the element here
      append = true;
    }
    ...
    // add headers
    if (!append && (type == 'h2' || type == 'h3' || type == 'h4')) {
      // add conditionals for the element here
      append = true;
    }
    ...
    if (nodeType === "div" && options.considerDIVs) {
      if (node.html().search(regexps.divToPElementsRe) === -1) {
        ...
        node.contents().each(function(index, element) {
          ...
          // handle noscript elements that has just text
          if ((childEntity.type == 'tag' && (childEntity.name == 'noscript') && childEntity.data)) {
            if (childEntity.data) {
              child.replaceWith('<p class="node-read-div2p">' + childEntity.data + '</p>');
            }
          }

          // handle i and em elements
          if ((childEntity.type == 'tag' && (childEntity.name == 'i' || childEntity.name == 'em') && childEntity.children)) {
            if (childEntity.children.length > 0) {
              if (childEntity.children[0].data) {
                child.replaceWith('<p class="node-read-div2p"><i>' + childEntity.children[0].data + '</i></p>');
              }
            }
          }
...
function getClassWeight(node) {
  ...
  if (node.get(0).name == "article") weight += 15;
