...
function filterCandidates(topCandidate, siblings, $) {
  ...
  siblings.each(function(index, elem) {
    ...
    if (!append && (type == 'p' || type == 'pre' || type == 'img')) {
    ...
    // add headers
    if (!append && (type == 'h2' || type == 'h3')) {
      append = true;
    }

    // convert divs? (may need to remove classes from children to avoid style breaking)
